          ...
          <div className="space-y-3">
            {predictions.map((prediction, idx) => {
              ...
              const getConfidenceColor = (conf: number): string => {
                if (conf >= 80) return 'bg-green-500/20 border-green-500/50 text-green-400';
                if (conf >= 70) return 'bg-yellow-500/20 border-yellow-500/50 text-yellow-400';
                if (conf >= 60) return 'bg-orange-500/20 border-orange-500/50 text-orange-400';
              ...
              return (
                ...
                >
                  <div className="flex items-start justify-between gap-4 mb-3">
                    <div className="flex-1">
                      <h3 className="text-lg font-semibold text-white">{predictionTitle}</h3>
                    ...
                    <div className={`px-3 py-1.5 rounded-full border font-bold text-sm whitespace-nowrap ${getConfidenceColor(confidencePercent)}`}>
                      {confidencePercent}%
                    </div>
                  </div>
